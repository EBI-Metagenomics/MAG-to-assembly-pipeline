...
def main(results: list, catalogue_metadata: Path, previous_table: Path, write_deleted: bool):
    ...
    expanded_df = combined_df.assign(
        Primary_assembly=combined_df["Primary_assembly"].str.split(",")
    ).explode("Primary_assembly")

    if catalogue_metadata:
        metadata_df = pd.read_csv(catalogue_metadata, sep="\t")
        merged_df = pd.merge(
            expanded_df,
            metadata_df[["Genome", "Species_rep", "Genome_accession"]],
            left_on="MAG_accession",
            right_on="Genome_accession",
            how="left",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create assembly-genome linking table from the output files of the main script"
    )
    parser.add_argument(
        "--previous-table",
        "-p",
